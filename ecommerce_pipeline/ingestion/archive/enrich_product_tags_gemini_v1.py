...
Uses Google Gemini API (free tier) to identify the most specific/meaningful tag 
...
OUTPUT_TABLE        = "product_tag_enriched"
...
def get_best_tag(title: str, category: str, tags: list) -> str:
    """Use Gemini to identify the most specific meaningful tag."""
...
Given a product's details, identify the MOST SPECIFIC and MEANINGFUL tag that best describes what the product actually is.
...
Rules:
- Pick the tag that is most specific to what the product IS (not just its broad category)
- Avoid generic terms that just repeat the category
- Return ONLY the chosen tag word/phrase, nothing else, no punctuation

Example: category=beauty, tags=["beauty","mascara"] → mascara
Example: category=groceries, tags=["pet supplies","cat food"] → cat food
Example: category=home-decoration, tags=["home decor","photo frame"] → photo frame"""
...
def main():
    print("=" * 60)
    print("Product Tag Enrichment — Powered by Gemini AI")
    print("=" * 60)
    ...
    query = f"""
        SELECT 
            ...
            category,
            tags
        FROM `{PROJECT_ID}.{INPUT_DATASET_ID}.{SOURCE_TABLE}`
        ORDER BY id
    """
